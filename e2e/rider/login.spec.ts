...
test.describe("Rider Login", () => {
  test("load /rider → login screen renders", async ({ page }) => {
    ...
    const heading = page.locator("h1, h2, [class*='text-2xl']").filter({ hasText: /Deliver with AJKMart/i }).first();
  ...
  test("home screen loads after login (online toggle visible)", async ({ page }) => {
    ...
    test.skip(!riderPhone || !riderPassword, "Set TEST_RIDER_PHONE and TEST_RIDER_PASSWORD env vars to run this test");
    ...
    const onlineToggle = page.locator(
