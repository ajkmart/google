...
test.describe("Vendor Orders", () => {
  test.beforeEach(async ({ page }) => {
    ...
    test.skip(!vendorPhone || !vendorPassword, "Set TEST_VENDOR_PHONE and TEST_VENDOR_PASSWORD to run vendor order tests");
  ...
  test("orders page loads with tabs (New / Active / All)", async ({ page }) => {
    await expect(page.locator("button, [role='tab']").filter({ hasText: /New|new/i }).first()).toBeVisible({ timeout: 10_000 });
    await expect(page.locator("button, [role='tab']").filter({ hasText: /Active|active/i }).first()).toBeVisible();
    await expect(page.locator("button, [role='tab']").filter({ hasText: /All|all/i }).first()).toBeVisible();
  });
  ...
  test("order card shows: ID, amount, customer name, status badge", async ({ page }) => {
    const orderCards = page.locator("[class*='border'], [class*='card'], [class*='rounded']").filter({ hasText: /PKR|Rs\.|#|Order/i });
