...
- Name: admin/categories.spec.ts >> Admin Categories >> fill name + select type → Save → new category appears in tree
- Location: e2e/admin/categories.spec.ts:40:7
...
Locator: locator('button').filter({ hasText: /Add Category/i }).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('button').filter({ hasText: /Add Category/i }).first()
