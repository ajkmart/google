...
import { Eye, EyeOff, Fingerprint, LogIn, Shield, Smartphone } from "lucide-react";
...
export default function LoginScreen({ onSuccess }: LoginScreenProps) {
  ...
  const passwordLogin = async () => {
    ...
    await handleLoginSuccess(result.data.token, { id: "", phone: identifier.trim(), roles: ["rider"] });
  };
  ...
  const verifyTwoFactor = async () => {
    ...
    try {
      const res = await api.twoFactorVerify({ code: twoFactorCode, tempToken: twoFactor.tempToken }) as Record<string, unknown>;
      ...
      <div style={{ minHeight: "100vh", background: theme.background, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
        <div style={cardStyle}>
          <div style={{ textAlign: "center", marginBottom: 18 }}>
            <div style={{ width: 60, height: 60, borderRadius: 18, margin: "0 auto 12px", background: `${theme.primary}18`, border: `1px solid ${theme.primary}40`, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Shield size={28} color={theme.primary} />
            </div>
            <h2 style={{ color: theme.text, fontSize: 20, fontWeight: 800, margin: 0 }}>Two-Factor Authentication</h2>
          ...
          <button onClick={() => setTwoFactor(null)} style={{ width: "100%", marginTop: 12, background: "none", border: "none", color: theme.textMuted, cursor: "pointer" }}>Back</button>
        </div>
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100vh", background: theme.background, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={cardStyle}>
        <div style={{ textAlign: "center", marginBottom: 22 }}>
          <div style={{ width: 72, height: 72, borderRadius: 22, margin: "0 auto 14px", background: `linear-gradient(135deg, ${theme.primary}, ${theme.primaryDark})`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 8px 28px ${theme.primary}40` }}>
            <Smartphone size={34} color="#fff" />
          </div>
          <h1 style={{ color: theme.text, fontSize: 26, fontWeight: 800, margin: 0 }}>Deliver with AJKMart</h1>
          <p style={{ color: theme.textMuted, fontSize: 13, margin: "6px 0 0" }}>Sign in to your rider account</p>
        </div>
        ...
        {mode === "otp" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <label style={{ display: "block", fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: theme.primary, marginBottom: 6 }}>Phone Number *</label>
              <div style={{ display: "flex", gap: 8 }}>
                <div style={{ height: 48, padding: "0 12px", background: theme.background, border: `1.5px solid ${theme.border}`, borderRadius: 12, display: "flex", alignItems: "center", fontSize: 13, fontWeight: 600, color: theme.textMuted }}>+92</div>
                <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="03XXXXXXXXX" maxLength={11} style={{ flex: 1, height: 48, padding: "0 16px", borderRadius: 12, background: theme.background, border: `1.5px solid ${theme.border}`, color: theme.text, fontSize: 14, outline: "none" }} />
              </div>
            </div>
            <button onClick={() => void sendPhoneOtp()} disabled={sending} style={{ width: "100%", height: 48, borderRadius: 12, border: "none", background: sending ? `${theme.primary}60` : `linear-gradient(135deg, ${theme.primary}, ${theme.primaryDark})`, color: "#fff", fontSize: 15, fontWeight: 700, cursor: sending ? "not-allowed" : "pointer" }}>
        ...
        {mode === "password" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <label style={{ display: "block", fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: theme.primary, marginBottom: 6 }}>Phone / Username *</label>
              <input value={identifier} onChange={e => setIdentifier(e.target.value)} placeholder="03XXXXXXXXX or username" style={{ width: "100%", height: 48, padding: "0 16px", borderRadius: 12, background: theme.background, border: `1.5px solid ${theme.border}`, color: theme.text, fontSize: 14, outline: "none" }} />
            </div>
            <div>
              <label style={{ display: "block", fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: theme.primary, marginBottom: 6 }}>Password *</label>
              <div style={{ position: "relative" }}>
                <input type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter your password" style={{ width: "100%", height: 48, padding: "0 16px", paddingRight: 44, borderRadius: 12, background: theme.background, border: `1.5px solid ${theme.border}`, color: theme.text, fontSize: 14, outline: "none" }} />
                <button type="button" onClick={() => setShowPassword(v => !v)} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: theme.textMuted, cursor: "pointer" }}>{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button>
              </div>
            </div>
            <div style={{ textAlign: "right", marginTop: -8 }}>
              <Link href="/forgot-password" style={{ fontSize: 12, color: theme.primary, textDecoration: "none", fontWeight: 600 }}>Forgot Password?</Link>
            </div>
            <button onClick={() => void passwordLogin()} disabled={signingIn} style={{ width: "100%", height: 48, borderRadius: 12, border: "none", background: signingIn ? `${theme.primary}60` : `linear-gradient(135deg, ${theme.primary}, ${theme.primaryDark})`, color: "#fff", fontSize: 15, fontWeight: 700, cursor: signingIn ? "not-allowed" : "pointer" }}>
        ...
        {(auth.googleEnabled || biometricAvailable) && (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {biometricAvailable && (
                <button onClick={() => void biometricSignIn()} disabled={biometricLoading} style={{ width: "100%", height: 44, borderRadius: 12, border: `1px solid ${theme.border}`, background: theme.background, color: theme.text, fontSize: 13, fontWeight: 600, cursor: biometricLoading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
        ...
        <p style={{ textAlign: "center", marginTop: 18, marginBottom: 0, fontSize: 13, color: theme.textMuted }}>
          New rider? <Link href="/register" style={{ color: theme.primary, fontWeight: 700, textDecoration: "none" }}>Create Account</Link>
        </p>
      </div>
    </div>
  );
