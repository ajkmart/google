...
export default function Chat() {
  ...
  const [tab, setTab] = useState<"chats" | "requests" | "search" | "ai">(() => {
    ...
    } catch (err) { log.error({ err: err instanceof Error ? err.message : String(err) }, "[Chat] URLSearchParams parse failed — defaulting to chats tab"); }
    return "chats";
  });
  ...
  useEffect(() => {
    ...
    } catch (err) { log.error({ err: err instanceof Error ? err.message : String(err) }, "[Chat] tab sync URLSearchParams parse failed"); }
  }, [search]);
  ...
  const loadConversations = useCallback(() => {
    api.apiFetch("/communication/conversations").then(setConversations).catch((err) => { log.error({ err: err instanceof Error ? err.message : String(err) }, "[Chat] loadConversations failed"); });
  }, []);

  const loadRequests = useCallback(() => {
    api.apiFetch("/communication/requests?type=received").then(setRequests).catch((err) => { log.error({ err: err instanceof Error ? err.message : String(err) }, "[Chat] loadRequests failed"); });
  }, []);

  const endCall = useCallback(() => {
    stopSound();
    if (callId) {
      api.apiFetch(`/communication/calls/${callId}/end`, { method: "POST", body: JSON.stringify({ duration: callTimer }) }).catch((err) => { log.error({ err: err instanceof Error ? err.message : String(err) }, "[Chat] endCall API failed"); });
  ...
  useEffect(() => {
    ...
    api.apiFetch("/communication/me/ajk-id").then(d => setAjkId(d.ajkId)).catch((err) => { log.error({ err: err instanceof Error ? err.message : String(err) }, "[Chat] fetchAjkId failed"); });
    loadConversations();
    loadRequests();
  ...
  const searchUser = async () => {
    ...
    } catch (err) { log.error({ err: err instanceof Error ? err.message : String(err) }, "[Chat] searchUser failed"); }
  };
    ...
    try {
      ...
      pc.ontrack = (e) => {
        if (remoteAudioRef.current) {
          ...
          remoteAudioRef.current.play().catch((err) => { log.error({ err: err instanceof Error ? err.message : String(err) }, "[Chat] remoteAudio.play (call) failed"); });
        }
      };
  ...
  const handleAcceptCall = async () => {
    try {
      ...
      pc.ontrack = (e) => {
        if (remoteAudioRef.current) {
          remoteAudioRef.current.srcObject = e.streams[0];
          remoteAudioRef.current.play().catch((err) => { log.error({ err: err instanceof Error ? err.message : String(err) }, "[Chat] remoteAudio.play (answer) failed"); });
        }
      };
  ...
  const sendAiMessage = async () => {
    ...
    } catch (err) { log.error({ err: err instanceof Error ? err.message : String(err) }, "[Chat] aiChat API failed"); }
    finally {
      setAiLoading(false);
