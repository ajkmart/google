...
import { ArrowLeft, Eye, EyeOff, Phone, Mail, CheckCircle, KeyRound } from "lucide-react";
...
function getPasswordStrength(pw: string): { level: number; label: TranslationKey; color: string; pct: string } {
  let score = 0;
...
const inputCss: React.CSSProperties = {
  ...
  background: T_.bg, border: `1.5px solid ${T_.border}`,
  color: T_.text, fontSize: 14, outline: "none", boxSizing: "border-box",
};
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      style={{ width: "100%", height: 48, borderRadius: 12, border: "none",
        background: disabled ? `${T_.primary}60` : `linear-gradient(135deg, ${T_.primary}, ${T_.primaryD})`,
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <input key={i} ref={el => { refs.current[i] = el; }}
          ...
          style={{
            ...
            color: T_.text, transition: "all 0.15s", boxSizing: "border-box",
          }}
        />
      ))}
    </div>
...
export default function ForgotPassword() {
  ...
  const pageStyle: React.CSSProperties = {
    minHeight: "100vh", background: T_.bg,
    display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
    padding: "24px 16px",
  };

  const cardStyle: React.CSSProperties = {
    background: T_.surface, border: `1px solid ${T_.border}`,
    ...
    return (
      <div style={pageStyle}>
        <div style={{ ...cardStyle, textAlign: "center" }}>
          <div style={{ width: 72, height: 72, borderRadius: "50%", background: `${T_.primary}15`, border: `2px solid ${T_.primary}40`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
            <CheckCircle size={36} color={T_.primary} />
          </div>
          <h2 style={{ color: T_.text, fontSize: 22, fontWeight: 800, margin: "0 0 8px" }}>{T("passwordResetSuccess")}</h2>
          <p style={{ color: T_.muted, fontSize: 14, lineHeight: 1.6, margin: "0 0 24px" }}>{T("passwordResetSuccessMsg")}</p>
          <Link href="/login" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, width: "100%", height: 48, borderRadius: 12, background: `linear-gradient(135deg, ${T_.primary}, ${T_.primaryD})`, color: "#fff", fontWeight: 700, fontSize: 15, textDecoration: "none" }}>
      ...
      <div style={pageStyle}>
        <div style={cardStyle}>
          <button onClick={() => setStep("new-password")}
            style={{ background: "none", border: "none", color: T_.muted, fontSize: 13, fontWeight: 600, cursor: "pointer", padding: 0, display: "flex", alignItems: "center", gap: 4, marginBottom: 16 }}>
    ...
    <div style={pageStyle}>
      ...
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        <div style={{
          ...
          margin: "0 auto 14px", boxShadow: `0 8px 28px ${T_.primary}45`,
        }}>
          <KeyRound size={32} color="#fff" />
        </div>
        <h1 style={{ color: T_.text, fontSize: 24, fontWeight: 800, margin: "0 0 4px" }}>{T("forgotPassword")}</h1>
      ...
      <div style={cardStyle}>
        ...
        {step !== "choose-method" && (
          <button onClick={goBack}
            style={{ background: "none", border: "none", color: T_.muted, fontSize: 13, fontWeight: 600, cursor: "pointer", padding: "0 0 16px", display: "flex", alignItems: "center", gap: 4 }}>
            <ArrowLeft size={14} /> {T("back")}
          </button>
        )}
        ...
        {/* Error banner */}
        {(error || isRateLimited) && (
          <div role="alert" style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 10, padding: "10px 14px", marginBottom: 16, color: "#fca5a5", fontSize: 13, fontWeight: 500 }}>
        ...
        {step === "choose-method" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <h3 style={{ color: T_.text, fontSize: 17, fontWeight: 700, margin: "0 0 4px" }}>{T("chooseResetMethod")}</h3>
            {hasPhoneOtp && (
              <button onClick={() => { setMethod("phone"); setStep("send-otp"); }}
                style={{ width: "100%", height: 56, border: `1.5px solid ${T_.border}`, borderRadius: 14, background: T_.bg, color: T_.text, fontSize: 14, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 14, padding: "0 16px", transition: "border-color 0.15s" }}
        ...
        {step === "send-otp" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {method === "phone" ? (
              <>
                <h3 style={{ color: T_.text, fontSize: 17, fontWeight: 700, margin: 0 }}>{T("resetViaPhone")}</h3>
                <label style={{ display: "block", fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: T_.primary }}>Phone Number *</label>
                <div style={{ display: "flex", gap: 8 }}>
                  <div style={{ height: 48, padding: "0 12px", background: T_.bg, border: `1.5px solid ${T_.border}`, borderRadius: 12, display: "flex", alignItems: "center", fontSize: 13, fontWeight: 600, color: T_.muted, whiteSpace: "nowrap" }}>+92</div>
                  <input type="tel" value={phone}
                    onChange={e => { let v = e.target.value.replace(/\D/g, ""); if (v.startsWith("92")) v = v.slice(2); if (v.startsWith("0")) v = v.slice(1); setPhone(v.slice(0, 10)); clearError(); }}
                    placeholder={phoneHint} onKeyDown={e => e.key === "Enter" && sendOtp()}
                    style={{ ...inputCss, flex: 1 }} autoFocus />
                </div>
              </>
            ) : (
              <>
                <h3 style={{ color: T_.text, fontSize: 17, fontWeight: 700, margin: 0 }}>{T("resetViaEmail")}</h3>
                <label style={{ display: "block", fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: T_.primary }}>Email Address *</label>
                <input type="email" value={email} onChange={e => { setEmail(e.target.value); clearError(); }}
                  placeholder="your@email.com" onKeyDown={e => e.key === "Enter" && sendOtp()}
                  style={inputCss} autoFocus />
              </>
            )}
            <BtnPrimary onClick={sendOtp} disabled={loading || isRateLimited}>
              {loading ? <><Spin /> {T("pleaseWait")}</> : isRateLimited ? `Try again in ${secondsLeft}s` : T("sendResetOtp")}
            </BtnPrimary>
          </div>
        )}
        ...
        {step === "enter-otp" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ textAlign: "center" }}>
              <h3 style={{ color: T_.text, fontSize: 17, fontWeight: 700, margin: "0 0 4px" }}>{T("enterResetOtp")}</h3>
              <p style={{ color: T_.muted, fontSize: 13, margin: 0 }}>{method === "phone" ? `+92 ${phone}` : email}</p>
            </div>
            {import.meta.env.DEV && devOtp && (
              <div style={{ background: "#1a2035", border: "1px solid #2d3a55", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "#94a3b8" }}>
                Dev OTP: <strong style={{ color: T_.primary }}>{devOtp}</strong>
              </div>
            )}
            <OtpBoxes value={otp} onChange={v => { setOtp(v); clearError(); }} />
            <BtnPrimary onClick={() => { if (otp.length >= 6) setStep("new-password"); else setError(T("enterOtpDigits")); }}>
              {T("nextStep")}
            </BtnPrimary>
            <button onClick={sendOtp} style={{ background: "none", border: "none", color: T_.muted, fontSize: 12, cursor: "pointer", padding: 0, textAlign: "center" }}>
              {T("resendOtp")}
            </button>
          </div>
        )}
        ...
        {step === "new-password" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <h3 style={{ color: T_.text, fontSize: 17, fontWeight: 700, margin: 0 }}>{T("newPassword")}</h3>
            <div>
              <label style={{ display: "block", fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: T_.primary, marginBottom: 6 }}>New Password *</label>
              <div style={{ position: "relative" }}>
                <input type={showPwd ? "text" : "password"} value={newPassword}
                  onChange={e => { setNewPassword(e.target.value); clearError(); }}
                  placeholder="Min 8 characters" style={{ ...inputCss, paddingRight: 44 }} autoFocus />
                <button type="button" onClick={() => setShowPwd(v => !v)}
                  style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: T_.muted, cursor: "pointer", padding: 0 }}>
                  {showPwd ? <EyeOff size={16} /> : <Eye size={16} />}
                </button>
              </div>
              {newPassword && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
                  <div style={{ flex: 1, height: 4, background: "#252D3A", borderRadius: 4, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: getPasswordStrength(newPassword).pct, background: getPasswordStrength(newPassword).color, borderRadius: 4, transition: "width 0.3s" }} />
                  </div>
                  <span style={{ fontSize: 10, fontWeight: 700, color: T_.muted }}>{T(getPasswordStrength(newPassword).label)}</span>
                </div>
              )}
            </div>
            <div>
              <label style={{ display: "block", fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: T_.primary, marginBottom: 6 }}>Confirm Password *</label>
              <input type={showPwd ? "text" : "password"} value={confirmPw}
                onChange={e => { setConfirmPw(e.target.value); clearError(); }}
                placeholder="Re-enter password" style={inputCss} />
              {confirmPw && newPassword !== confirmPw && (
                <p style={{ color: "#ef4444", fontSize: 11, margin: "4px 0 0" }}>{T("passwordsDoNotMatch")}</p>
              )}
            </div>
            <BtnPrimary onClick={() => verifyOtpAndSetPassword()} disabled={loading}>
              {loading ? <><Spin /> {T("pleaseWait")}</> : T("resetPassword")}
            </BtnPrimary>
          </div>
        )}
        ...
        <div style={{ textAlign: "center", marginTop: 20 }}>
          <Link href="/login" style={{ fontSize: 13, color: T_.primary, fontWeight: 600, textDecoration: "none" }}>
            {T("backToLogin")}
          </Link>
        </div>
      </div>
