...
export function LoginScreen({ onSuccess }: LoginScreenProps) {
  ...
  const handleSuccess = useCallback(async (_sdkUser: SDKAuthUser, _sdkToken: string) => {
    ...
    if (!profileRoles.includes("vendor")) {
      api.clearTokens();
      setLoginError(T("accessDenied") as string || "Access denied. This app is for vendors only.");
      setIsProcessing(false);
      return;
    }
  ...
  const handleFacebook = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.socialFacebook({ accessToken, deviceMeta }) as Record<string, unknown>;
      api.storeTokens(res.token as string, res.refreshToken as string | undefined);
      await handleSuccess(res.user as SDKAuthUser, res.token as string);
    } catch (e: unknown) { setLoginError(e instanceof Error ? translateApiError(e.message) : T("loginFailed") as string); }
  ...
  const btnPrimary: React.CSSProperties = {
    ...
    color: "#fff", fontSize: 15, fontWeight: 700, cursor: isBlocked ? "not-allowed" : "pointer",
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.background, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 16px", pointerEvents: isBlocked ? "none" : "auto" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        ...
        <h1 style={{ color: theme.text, fontSize: 26, fontWeight: 800, margin: "0 0 4px", letterSpacing: "-0.5px" }}>AJKMart Vendor</h1>
        <p style={{ color: theme.textMuted, fontSize: 13, margin: 0 }}>Sign in to your vendor account</p>
      </div>
        ...
        {loginMode === "otp" && otpStep === "phone" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <button onClick={() => void handleSendOtp()} disabled={otpSending} style={btnPrimary}>
              {otpSending ? <><Spinner color="#fff" /> Sending OTP…</> : T("sendOtpBtn") as string || "Send OTP"}
            </button>
          </div>
        )}
        ...
        {loginMode === "otp" && otpStep === "otp" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <button onClick={() => void handleVerifyOtp()} disabled={verifying || localOtp.length < 6} style={btnPrimary}>
              {verifying ? <><Spinner color="#fff" /> Verifying…</> : "Verify & Sign In"}
            </button>
        ...
        {loginMode === "password" && (
          <form onSubmit={e => void handlePasswordLogin(e)} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <button type="submit" disabled={signingIn} style={btnPrimary}>
              {signingIn ? <><Spinner color="#fff" /> Signing In…</> : T("signIn") as string || "Sign In"}
            </button>
          </form>
        )}
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: theme.background, padding: 16 }}>
      <div style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 18, padding: "28px 24px", boxShadow: "0 20px 60px rgba(0,0,0,0.5)", width: "100%", maxWidth: 384, textAlign: "center" }}>
        ...
        <p style={{ color: theme.textMuted, fontSize: 14, lineHeight: 1.6, margin: "0 0 20px" }}>Your {appName ?? "vendor application"} is being reviewed by our team. You'll be notified once approved.</p>
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: theme.background, padding: 16 }}>
      <div style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 18, padding: "28px 24px", boxShadow: "0 20px 60px rgba(0,0,0,0.5)", width: "100%", maxWidth: 384, textAlign: "center" }}>
        ...
        <button onClick={onAccept} disabled={loading} style={{ background: loading ? `${theme.primaryDark}80` : `linear-gradient(135deg, ${theme.primary}, ${theme.primaryDark})`, color: "#fff", border: "none", borderRadius: 12, padding: "12px 20px", fontSize: 15, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", width: "100%", marginBottom: 10, opacity: loading ? 0.7 : 1 }}>
