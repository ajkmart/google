...
router.post("/send-otp", otpLimiter, verifyCaptcha, validateBody(SendOtpSchema), async (req, res) => {
  try {
    ...
    if (!isAuthMethodEnabled(settings, "auth_phone_otp_enabled")) {
      sendErrorWithData(res, "Phone OTP is currently disabled. Please use another login method.", { code: "AUTH_METHOD_DISABLED" }, 400);
      return;
    }

    // ── Look up existing user (server-side only — never exposed in response) ──
    const [existingUser] = await db.select({ id: usersTable.id, roles: usersTable.roles, isBanned: usersTable.isBanned, googleId: usersTable.googleId })
      .from(usersTable).where(eq(usersTable.phone, phone)).limit(1);

    const effectiveRole = existingUser?.roles ?? (req.body.role === "rider" || req.body.role === "vendor" ? req.body.role : "customer");

    if (!isAuthMethodEnabled(settings, "auth_phone_otp_enabled", effectiveRole)) {
      sendErrorWithData(res, "Phone OTP is currently disabled for your account type.", { code: "AUTH_METHOD_DISABLED" }, 400);
      return;
    }
    ...
    if (existingUser?.googleId && isAuthMethodEnabled(settings, "auth_google_enabled", effectiveRole)) {
    ...
    if (err instanceof OtpBlockedError) {
      sendTooManyRequests(res, err.message); return;
    }
...
router.post("/verify-otp", otpLimiter, verifyCaptcha, validateBody(VerifyOtpSchema), async (req, res) => {
  try {
    const phone  = canonicalizePhone(req.body.phone);
    ...
    try {
      ...
      if (err instanceof OtpExpiredError) {
        sendErrorWithData(res, "OTP expired or not found. Please request a new one.", {}, 401); return;
      }
      throw err;
    }
    ...
    if (verified.isNewUser) {
      ...
      if (signupBonus > 0) {
        await db.update(usersTable).set({ walletBalance: sql`wallet_balance + ${signupBonus}` }).where(eq(usersTable.id, newUserId));
      ...
      sendSuccess(res, {
        ...
        user: {
          id: newUserId, phone, name: null, email: null, username: null,
          roles: "customer", walletBalance: signupBonus,
          isActive: !requireApproval, totpEnabled: false,
        },
    ...
    const isCustomerAppCtx = requestedRole === "customer" || appIdHeader === "customer" || appIdQuery === "customer";
    ...
    if (user.isBanned) {
      addSecurityEvent({ type: "banned_login_attempt", ip, userId: user.id, details: `Banned user verified OTP: ${phone}`, severity: "high" });
      sendForbidden(res, "Your account has been suspended. Please contact support.");
      return;
    }

    if (user.googleId && isAuthMethodEnabled(settings, "auth_google_enabled", user.roles ?? undefined)) {
      addSecurityEvent({ type: "otp_hijack_google_account", ip, userId: user.id, details: `OTP verify on Google-linked account: ${phone}`, severity: "medium" });
    ...
    if (!user.isActive && !isPendingApproval) {
      sendForbidden(res, "Your account is currently inactive. Please contact support."); return;
    }
    ...
    if (user.totpEnabled && isAuthMethodEnabled(settings, "auth_2fa_enabled", user.roles ?? undefined)) {
      ...
      if (!isDeviceTrusted(user, deviceFingerprint ?? "", trustedDays)) {
        const tempToken = sign2faChallengeToken(user.id, user.phone ?? "", user.roles ?? "customer", user.roles ?? "customer", "phone_otp");
    ...
    if (isPendingApproval) {
      ...
      sendSuccess(res, {
        token: accessToken, expiresAt: new Date(Date.now() + getAccessTokenTtlSec() * 1000).toISOString(),
    ...
    if (isCustomerAppCtx && !userRoles.includes("customer")) {
      addSecurityEvent({ type: "cross_role_login_attempt", ip, userId: user.id, details: `User [${user.roles}] phone-logged in to customer app — offering add-role`, severity: "low" });
    }

    // Issue full session
    const tokens = await issueTokensForUser(user, ip, "phone_otp", req.headers["user-agent"] as string | undefined, req, res);
    sendSuccess(res, {
      ...tokens,
      isNewUser: false,
      ...(isCustomerAppCtx && !userRoles.includes("customer") ? { canAddCustomerRole: true, code: "cross_app_account", wrongApp: true } : {}),
    });
  } catch (err) {
    logger.error({ error: err instanceof Error ? err.message : String(err) }, "[phone.routes] verify-otp error");
    res.status(500).json({ success: false, error: "Internal server error" });
  }
});
