...
router.post("/send-email-otp", emailOtpLimiter, verifyCaptcha, validateBody(SendEmailOtpSchema), async (req, res) => {
  try {
    ...
    const [user] = await db.select({ id: usersTable.id, roles: usersTable.roles, isBanned: usersTable.isBanned, isActive: usersTable.isActive, approvalStatus: usersTable.approvalStatus })
      .from(usersTable).where(eq(usersTable.email, normalized)).limit(1);

    // Non-enumeration: if no user, return generic success — don't reveal whether the email is registered
    if (!user) {
      sendSuccess(res, { message: "If an account exists with this email, an OTP has been sent.", channel: "email" });
      return;
    }
    ...
    if (!user.isActive && !isPending) { sendForbidden(res, "Your account is inactive. Contact support."); return; }
...
router.post("/verify-email-otp", emailOtpLimiter, verifyCaptcha, validateBody(VerifyEmailOtpSchema), async (req, res) => {
  try {
    const normalized = (req.body.email as string).toLowerCase().trim();
    ...
    if (!userId) {
      sendNotFound(res, "No account found with this email address."); return;
    }
    ...
    if (requestedRole && !isCustomerAppCtx) {
      ...
      if (!userRoles.includes(requestedRole)) {
        addSecurityEvent({ type: "cross_role_login_attempt", ip, userId: user.id, details: `User [${user.roles}] tried email OTP login as ${requestedRole}`, severity: "high" });
        sendErrorWithData(res, `This account is not registered as a ${requestedRole}. Please use the correct app.`, { wrongApp: true }, 403); return;
      }
    }

    if (user.isBanned) { sendForbidden(res, "Account suspended. Contact support."); return; }
    const isPending = user.approvalStatus === "pending";
    if (!user.isActive && !isPending) { sendForbidden(res, "Account inactive. Contact support."); return; }
    ...
    // Mark email verified
    await db.update(usersTable).set({ emailVerified: true, lastLoginAt: new Date(), updatedAt: new Date() }).where(eq(usersTable.id, user.id));
    ...
    if (isPending) {
      AuditService.log({ action: "email_login", ip, details: `Email OTP login (pending): ${normalized}`, result: "success" });
      // Issue access-only token for pending accounts
      const { signAccessToken, getAccessTokenTtlSec: ttl } = await import("../../middleware/security.js");
      ...
      sendSuccess(res, {
        ...
        pendingApproval: true, message: "Aapka account admin approval ke liye bheja gaya hai.",
    ...
    AuditService.log({ action: "email_login", ip, details: `Email OTP login: ${normalized}`, result: "success" });
    ...
    sendSuccess(res, {
      ...tokens,
      ...(isCrossApp ? { canAddCustomerRole: true, code: "cross_app_account", wrongApp: true } : {}),
    });
  } catch (err) {
    logger.error({ error: err instanceof Error ? err.message : String(err) }, "[email.routes] verify-email-otp error");
    res.status(500).json({ success: false, error: "Internal server error" });
  }
});
