...
import { liveLocationsTable, locationLogsTable, ridesTable, ordersTable, usersTable, riderProfilesTable } from "@workspace/db/schema";
...
async function broadcastRiderLocation(userId: string, lat: number, lon: number, opts: {
  ...
  /* Fetch rider's vehicle type from rider_profiles (for map markers) */
  try {
