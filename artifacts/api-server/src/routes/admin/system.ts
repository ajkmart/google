...
router.get("/auth/methods", adminAuth, async (_req, res, next) => {
  try {
    ...
    const keys = ["auth_phone_otp_enabled", "auth_email_otp_enabled", "auth_username_password_enabled", "auth_google_enabled", "auth_facebook_enabled", "auth_magic_link_enabled", "auth_biometric_enabled", "auth_totp_enabled"] as const;
    ...
    for (const key of keys) {
      const parsed = (() => {
        try {
          ...
          return { customer: settings[key] ?? "off", rider: settings[key] ?? "off", vendor: settings[key] ?? "off" };
        }
      })();
      methods[key] = {
...
router.patch("/settings/auth-methods", adminAuth, async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(body.settings)) { sendValidationError(res, "settings array required"); return; }
    await db.insert(platformSettingsTable).values(body.settings.map(({ key, value }) => ({ key, value: String(value), label: key, category: "auth", updatedAt: new Date() }))).onConflictDoUpdate({ target: platformSettingsTable.key, set: { value: sql`excluded.value`, updatedAt: sql`excluded.updated_at` } });
    invalidateSettingsCache();
    invalidatePlatformSettingsCache();
    addAuditEntry({ action: "auth_methods_update", ip: getClientIp(req), adminId: (req as AdminRequest).adminId, details: `Updated ${body.settings.length} auth settings`, result: "success" });
...
router.get("/auth/events", adminAuth, async (req, res, next) => {
  try {
    ...
    const rows = await db.select({ id: authEventsTable.id, timestamp: authEventsTable.createdAt, user: usersTable.name, event_type: authEventsTable.eventType, channel: authEventsTable.channel, role: authEventsTable.role, success: authEventsTable.success, ip: authEventsTable.ip }).from(authEventsTable).leftJoin(usersTable, eq(authEventsTable.userId, usersTable.id)).where(conditions.length ? and(...conditions) : undefined).orderBy(desc(authEventsTable.createdAt)).limit(limit);
...
router.get("/auth/locked-users", adminAuth, async (_req, res, next) => {
  try {
    ...
    const rows = await db.select({ id: usersTable.id, phone: usersTable.phone, email: usersTable.email, attempts: otpAttemptsTable.count, locked_since: otpAttemptsTable.firstAt }).from(otpAttemptsTable).leftJoin(usersTable, eq(otpAttemptsTable.key, usersTable.phone)).where(gte(otpAttemptsTable.count, maxAttempts)).orderBy(desc(otpAttemptsTable.count));
    sendSuccess(res, { users: rows.map(r => ({ ...r, locked_since: r.locked_since?.toISOString() ?? null })) });
  } catch (err) { next(err); }
});
