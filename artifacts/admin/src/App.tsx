...
function ProtectedRoute({ component: Component, requirePermission, fullScreen = false }: { component: React.ComponentType; requirePermission?: string | string[]; fullScreen?: boolean; }) { const { hasPermission } = usePermissions(); const allowed = !requirePermission || hasPermission(requirePermission); if (!allowed) return <RedirectTo to="/403" />; return fullScreen ? <Component /> : <AdminLayout><Component /></AdminLayout>; }
